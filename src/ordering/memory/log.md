...
2026-07-15 | 0.9124 → 0.9102 | widened Scotch to its own tier (n<60k) — 1k_10k bucket 0.9364→0.9274 — then re-budgeted all bands from per-candidate µs/nnz measurements under CPU contention: Scotch band capped at nnz<70k, METIS 150k, extra partitioner seeds confined to nnz<30k, tiny tier 25k | win — score 0.9102 with worst local order() ≈0.5-0.8 s measured on a LOADED box (6 competing procs); envelope math: ≈5.6 µs/nnz worst-case stack → ≈0.4 s at band top quiet, ≈2× safety vs cap at 5x-slower grader is thinner than before — do NOT widen bands further without per-band time re-measurement on quiet hardware (see [0002](experiments/0002-tiered-best-of-portfolio.md) addendum)
